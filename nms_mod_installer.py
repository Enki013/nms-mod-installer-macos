...
DEFAULT_GAME_PATH = "/Users/{user}/Desktop/No Man's Sky.app".format(
...
def resolve_game_path(game_path):
...
def scan_mod(mod_folder: Path, banks_dir: Path, tool_path: str):
    ...
    pak_map = defaultdict(list)  # pak_name -> [(mod_file_path, internal_path)]
    unmatched = []

    for full_path, internal_path in mod_files:
        pak = index.find_pak(internal_path)
        if pak:
            pak_map[pak].append((full_path, internal_path))
        else:
...
def install_mod(mod_folder: Path, game_path: Path, banks_dir: Path, tool_path: str,
    ...
    with tempfile.TemporaryDirectory(prefix="nms_mod_") as tmpdir:
        ...
        for pak_name, files in sorted(pak_map.items()):
            ...
            # Replace files
            replaced = 0
            for mod_file, internal_path in files:
...
def list_mods(banks_dir: Path):
    ...
    for name, meta in mods.items():
        print(f"  {Style.CYAN}{name}{Style.RESET}")
        ...
        print(f"    Source:        {meta.get('source_folder', '?')}")
        print()
...
examples:
  ...
  %(prog)s uninstall "Turkish Localisation"
  %(prog)s list
  %(prog)s scan ./MyMod
        """,
    )
    ...
    p_install.add_argument("--game", default=DEFAULT_GAME_PATH, help="Path to No Man's Sky.app")
    ...
    # uninstall
    p_uninstall = sub.add_parser("uninstall", help="Uninstall a mod by name")
    p_uninstall.add_argument("mod_name", help="Name of the mod to uninstall")
    p_uninstall.add_argument("--game", default=DEFAULT_GAME_PATH, help="Path to No Man's Sky.app")

    # list
    p_list = sub.add_parser("list", help="List installed mods")
    p_list.add_argument("--game", default=DEFAULT_GAME_PATH, help="Path to No Man's Sky.app")
    ...
    p_scan.add_argument("--game", default=DEFAULT_GAME_PATH, help="Path to No Man's Sky.app")
    ...
    # Resolve game path
    game_path, banks_dir = resolve_game_path(args.game)
    ...
    if args.command == "install":
        ...
        uninstall_mod(args.mod_name, banks_dir)
