#!/usr/bin/env python3
"""
NMS Mod Installer for macOS — PyPI package entry point
============================
...
def run_mbincompiler(input_path: Path, output_dir: Path) -> Optional[Path]:
    ...
    macOS: uses the self-contained binary directly  (./MBINCompiler file)
    ...
    with tempfile.TemporaryDirectory(prefix="mbinc_") as tmpdir:
        ...
        if is_mxml:
            # Ensure extension is .MXML (MBINCompiler expects this for MBIN output)
